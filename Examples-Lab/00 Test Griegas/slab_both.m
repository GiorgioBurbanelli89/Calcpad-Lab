%% Diferencia CON vs SIN unidades, medido en la MISMA corrida (justo)
% #plain
un=symunit;
a=6; b=4; t=0.1; q=10; E_si=35000*1000; nu=0.15;
...
N=50;
% --- SIN unidades ---
tic; for it=1:N, wc=analiza(D,gp,gw,n_gp,a_1,b_1,q,n_e,n_j,n_g,e_j,s_j,x_j,y_j,b,a,n_a,n_b); end; t_sin=toc;
% --- CON unidades (misma analiza numerica + salida con unidad por iter) ---
tic; for it=1:N, wc=analiza(D,gp,gw,n_gp,a_1,b_1,q,n_e,n_j,n_g,e_j,s_j,x_j,y_j,b,a,n_a,n_b); w_u=(wc*1000)*un.mm; end; t_con=toc;
fprintf('SIN unidades: %.4f ms/iter\n', 1000*t_sin/N);
fprintf('CON unidades: %.4f ms/iter\n', 1000*t_con/N);
fprintf('overhead unidades: %.1f%%\n', 100*(t_con-t_sin)/t_sin);
